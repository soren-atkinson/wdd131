...
function search() {
    ...
    let filteredRecipes = recipes.filter(function(item){
        return ( 
            item.name.toLowerCase().includes(recipeQuery.toLowerCase()) ||
            item.description.toLowerCase().includes(recipeQuery.toLowerCase()) || 
            item.tags.find(tag => tag.toLowerCase().includes(recipeQuery.toLowerCase()))
        );
    })
    ...
    sortedRecipes.forEach(function(item){
        renderRecipe(item);
      })
}
...
function tagTemplate(tags) {
    return `
        <div class="categories">
            ${tags.map(tag => `<button class="category">${tag}</button>`).join('')}
        </div>
    `;
}
